...
def train_data():
    ...
    for se in data_set:
        ...
        while (epoch + batch_size <= len(x_train)):
            for step in range(FLAGS.epochs):
                ...
                if (epoch * FLAGS.epochs /20 + step) % 10 == 0:
                    ...
                    print("Epoch:", '%04d' % (epoch * FLAGS.epochs / 20 + step + 1), "t_los=",
    ...
    pd.DataFrame(acc_list,columns=['loss1','acc','loss2']).to_csv('his_d/acc'+str(sjd)+'-5.csv',index=None)
